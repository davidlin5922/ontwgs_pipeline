...
`bamqc`, `qc`, and `sniffles2` all contains folders named by the same_names you gave in which is where the tables and figures in `report.html` originated. `bams` contains all the aligned, sorted, and indexed bam files for you.
...
`nanoplot` is used to assess the quality of your sequencing input.

2.  Alignment

`minimap2` is used to align the sequences to the reference genome of your choice.

3.  Structural Variant Calling

`sniffles2` is used to do structural variant calling.

4.  Result Visualization

All tables and figures are consolidated in `report.html` with comparisons in QC metrics and SV callings.

### Sample Run

A toy dataset is included in `example/`. The two fastq files each contains 1000 reads from the SiHa cell line, isolated from fragments of a primary uterine tissue sample from a 55-year-old, female, Japanese patient with squamous cell carcinoma. 
